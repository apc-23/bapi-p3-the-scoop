...
//console.log(`start articleId: ${requestComment.articleId}`);
//console.log(requestComment);
  ...
  //console.log(`sts = ${response.status}`);
  return response;
};
...
function loadDatabase() {
  try {
    doc = yaml.safeLoad(fs.readFileSync('scoop.yml', 'utf8'));
    if (doc){
      database = doc;
    }
...
function saveDatabase() {
  ...
  fs.writeFile('scoop.yml',yaml.safeDump (database));
  return true;
};
